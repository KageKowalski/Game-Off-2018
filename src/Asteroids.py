...
class Asteroids:
    ...
    def draw(self):
        if self.game_start:
            pyxel.cls(0)
            pyxel.text(8,8,"Welcome to Asteroids!",8)
            pyxel.text(8,24,"Avoid getting hit by asteroids.",8)
            pyxel.text(8,32,"WASD - Movement | Mouse - Select Upgrade",8)
            pyxel.text(8,48,"Press 'E' to continue.",8)
        else:
            ...
            if not self.ch_upg:
                ...
                pyxel.text((pyxel.width / 2) + 25, (pyxel.height / 2) + 6,"Nothing", 15)
